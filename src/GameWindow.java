...
public class GameWindow extends JPanel implements MouseListener,
	...
	public void paint(Graphics g) {
		...
		if (rightEdge.inside(ball.getLocation().getIntX(), ball.getLocation().getIntY()) || rightEdge.inside(ball2.getLocation().getIntX(), ball2.getLocation().getIntY())){
			System.out.println("Game over");
			g2d.drawString("Game over", 200, 400);
		}
		
		if (rightPaddle.Inside(new doublePoint(ball.getLocation().getX() + 30, ball.getLocation().getY() + 30))){ //acounting for the ball exmpading from location point
	...
	private void initEdgeDetectors() {
		rightEdge = new Rectangle(0,0,1,mainFrame.getHeight()); //draws a single pixel wide barrier
		leftEdge = new Rectangle(1400,0,1,mainFrame.getHeight());
		
		
	}
	
	private void initPaddles(){
		leftPaddle = new leftPaddle(this, new doublePoint(0,35), new doublePoint(20,100), Color.BLUE);
		rightPaddle = new rightPaddle(this, new doublePoint(1400, 0), new doublePoint(20,1050), Color.RED);
		this.repaint();
	}
